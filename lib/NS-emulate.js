...
export class NS {
    constructor(args) {
        this.implemented_functions = Object.getOwnPropertyNames(NS.prototype).filter(function (p) {
            return typeof NS.prototype[p] === 'function' && p !== 'constructor'
        })
        this.description = `Emulates Bitburner NS interface to test code locally. Currently implemented functions: ${this.implemented_functions}`;
        this.args = args;
    }
    ...
    async wget(url, file_path) {
        ...
        if (!response.ok) {
            // Match ns.wget behaviour in case of failure, return false and log error, rather than throw an Error
            // throw new DownloadError(`Unable to download the file (${response.status})`);
